...
try{
	// DATEADD(WEEK, -1, GETUTCDATE())
	$query = "SELECT user_symptom.id as id, name, pain_level, daily_reflection, note, user_symptom.date as date, TIMEDIFF(end_time, start_time) as pain_duration FROM user_symptom INNER JOIN symptom ON symptom_id = symptom.id WHERE user_symptom.user_id = $user_id ORDER BY user_symptom.date DESC";
	$result = $db->query($query);
	if($result){
		$symptoms = $result->fetch_all(MYSQLI_ASSOC);
